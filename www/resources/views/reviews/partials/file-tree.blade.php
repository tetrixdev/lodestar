@php
    ...
    // Map each changed-file id -> the 1-based section numbers covering it.
    // Keyed by id (not path) so a path that appears in two repos stays distinct.
    $coverMap = [];
    ...
    $cov = $review->coverage();
    $multiRepo = $review->comparisons->count() > 1;
    $statusColor = [
...
<div class="bg-white border border-gray-200 rounded-lg" x-data="{ open: {{ $cov['complete'] ? 'false' : 'true' }} }">
    <header class="flex items-center gap-3 px-4 py-3 cursor-pointer" @click="open = !open">
        ...
        <span class="text-xs text-gray-500">{{ $cov['total'] }} file{{ $cov['total'] === 1 ? '' : 's' }}@if ($multiRepo) across {{ $review->comparisons->count() }} repos @endif</span>
        @if ($cov['complete'])
    ...
    <div x-show="open" x-collapse x-cloak class="border-t border-gray-100">
        @foreach ($review->comparisons as $comparison)
            {{-- A per-repo subheading only when the review spans more than one comparison. --}}
            @if ($multiRepo)
                <div class="px-4 py-1.5 bg-gray-50 border-t border-gray-100 first:border-t-0 text-[11px] font-medium text-gray-500">
                    {{ $comparison->repository?->full_name ?? 'comparison' }}
                    <span class="text-gray-400">{{ $comparison->base_ref ?? '?' }} … {{ $comparison->head_ref ?? '?' }}</span>
                </div>
            @endif
            <ul class="divide-y divide-gray-50 font-mono text-xs">
                @foreach ($comparison->files as $f)
                    @php $covering = $coverMap[$f->id] ?? []; @endphp
                    <li>
                        <x-open-file :file="$f"
