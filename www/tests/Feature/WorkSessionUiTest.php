...
class WorkSessionUiTest extends TestCase
{
    ...
    public function test_creating_a_session_with_a_task_links_it(): void
    {
        $user = User::factory()->create();
        $project = $this->project($user);
        $task = $project->tasks()->create(['title' => 'Some task', 'status' => Task::STATUS_READY_FOR_PLANNING, 'position' => 0]);
    ...
    public function test_task_must_belong_to_the_project(): void
    {
        ...
        $foreignTask = $otherProject->tasks()->create(['title' => 'Elsewhere', 'status' => Task::STATUS_READY_FOR_PLANNING, 'position' => 0]);
